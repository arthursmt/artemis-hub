...
- Body: `proposalPayloadSchema` from `artemis-contracts`
- Response:
  - `proposalId`
  - `stage` (initial DOC_REVIEW)
  - `submittedAt`
...
- Body: `decisionRequestSchema` from `artemis-contracts`
- Response:
  - `previousStage`
  - `newStage`
  - `decision`
...
Hunt must be able to query proposals that returned for correction.
...
> These endpoints can be implemented in Arise reusing the same proposal storage.
